...
class Burgers1D1tBuilder(Builder):
    name = 'burgers_1t'
    ...
    def __init__(self, data_path: str, train_size: int, test_size: int,
        ...
        with h5py.File(os.path.expandvars(self.data_path), 'r') as f:
            data = np.swapaxes(f['tensor'],1,-1).astype(np.float32)[:train_size+2*test_size,...,np.array([self.Tinit,self.Tfinal])]
        ...
        self.test_dataset = Burgers1DDataset(
            data[:test_size])
        print(len(self.test_dataset))
               
        self.val_dataset = Burgers1DDataset(
            data[test_size:2*test_size])
        print(len(self.val_dataset))
        
        self.train_dataset = Burgers1DTrainingDataset(
...
class Burgers1DTrainingDataset(Dataset):
