...
class Darcy1tBuilder(Builder):
    ...
    def __init__(self, data_path: str, train_size: int, test_size: int,
        ...
        with h5py.File(os.path.expandvars(self.data_path), 'r') as f:
            data = np.stack((f['nu'],f['tensor'][:,0,...]),-1)#.astype(np.float32)
        ...
        self.test_dataset = DarcyDataset(
            data[:test_size])
        print(len(self.test_dataset))
        
        self.val_dataset = DarcyDataset(
            data[test_size:2*test_size])
        print(len(self.val_dataset))
        
        self.train_dataset = DarcyTrainingDataset(
...
class DarcyTrainingDataset(Dataset):
