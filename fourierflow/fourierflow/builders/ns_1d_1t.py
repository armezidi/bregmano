...
class NS1D1tBuilder(Builder):
    ...
    def __init__(self, data_path: str, train_size: int, test_size: int,
        ...
        with h5py.File(os.path.expandvars(self.data_path), 'r') as f:
            data = np.swapaxes(f['Vx'],1,-1).astype(np.float32)[:train_size+2*test_size,...,np.array([self.Tinit,self.Tfinal])]
        ...
        self.test_dataset = NavierStokes1DDataset(
            data[:test_size])
        print(len(self.test_dataset))
               
        self.val_dataset = NavierStokes1DDataset(
            data[test_size:2*test_size])
        print(len(self.val_dataset))
        
        self.train_dataset = NavierStokes1DTrainingDataset(
    ...
    def inference_data(self):
        with h5py.File(os.path.expandvars(self.data_path), 'r') as f:
            data = np.swapaxes(f['Vx'],1,-1).astype(np.float32)[:1,...,np.array([self.Tinit,self.Tfinal])]
          
        return {'data': data}


class NavierStokes1DTrainingDataset(Dataset):
    def __init__(self, data):
        ...
        x = rearrange(x, 'b m t -> (b t) m 1')
        y = rearrange(y, 'b m t -> (b t) m 1')
