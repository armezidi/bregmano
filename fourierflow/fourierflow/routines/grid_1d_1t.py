...
class Grid1D1tExperiment(Routine):
    def __init__(self,
                 conv: nn.Module,
    ...
    def encode_positions(self, dim_sizes, low=-1, high=1, fourier=True):
        ...
        fourier_feats = fourier_encode(pos, self.k_max, self.num_freq_bands, base=self.freq_base)
    ...
    def _build_features(self, batch):
        ...
        if self.use_position:
            pos_feats = self.encode_positions(dim_sizes, self.low, self.high, self.use_fourier_position)
            pos_feats = repeat(pos_feats, '... -> b ...', b=B)
            x = torch.cat([x, pos_feats], dim=-1)

        if self.append_force:
            f = repeat(batch['f'], 'b m -> b m 1')
            x = torch.cat([x, f], dim=-1)

        if self.append_mu:
            mu = repeat(batch['mu'], 'b -> b m 1', m=X)
    ...
    def _valid_step(self, batch):
        ...
        if self.append_force:
            if len(batch['f'].shape) == 2:
                force = repeat(batch['f'], 'b m -> b m t 1', t=xx.shape[-2])
            elif len(batch['f'].shape) == 3:
                f = batch['f'][..., -n_steps:]
                force = repeat(f, 'b m t -> b m t 1')

            xx = torch.cat([xx, force], dim=-1)

        if self.append_mu:
            mu = repeat(batch['mu'], 'b -> b m t 1', m=X, t=xx.shape[-2])
    ...
    def compute_losses(self, batch, loss, preds):
        ...
        loss /= n_steps
        loss_full = self.l2_loss(preds.reshape(B, -1), yy.reshape(B, -1))

        pred_norm = torch.norm(preds, dim=[1], keepdim=True)
        yy_norm = torch.norm(yy, dim=[1], keepdim=True)
        p = (preds / pred_norm) * (yy / yy_norm)
        p = p.sum(dim=[1]).mean(dim=0)

        has_diverged = p < 0.95
        diverged_idx = has_diverged.nonzero()
        diverged_t = diverged_idx[0, 0] if len(diverged_idx) > 0 else len(has_diverged)
        ...
        if 'corr_data' in batch:
            ...
            if dim_sizes[0] != corr_size:
                ...
                pred_2_norm = torch.norm(preds_2, dim=[1], keepdim=True)
                corr_yy_norm = torch.norm(corr_yy, dim=[1], keepdim=True)
                p_2 = (preds_2 / pred_2_norm) * (corr_yy / corr_yy_norm)
                p_2 = p_2.sum(dim=[1]).mean(dim=0)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss, loss_full, time_until, reduced_time_until, p, times = self.compute_losses(batch, loss, preds)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.pred_path:
            ...
            B, X, T = vorticities.shape
            vorticities_hat = jnp.fft.rfftn(vorticities, axes=(1,))

            sim_grid = Grid(shape=[X], domain=self.domain)
            out_grid = Grid(shape=[64], domain=self.domain)
            ...
            for b in range(B):
                ...
                for t in range(T):
                    ...
                    if X > 64:
                        ...
                        vxhat = velocity_solve(vorticity_hat)
                        vxb.append(jnp.fft.irfftn(vxhat, axes=(0,)))
