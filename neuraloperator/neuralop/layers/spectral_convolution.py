...
class SpectralConv(BaseSpectralConv):
    ...
    def forward(
        ...
        # print(type(mode_sizes))
        # print(f"{mode_sizes=}")
        # print(f"{mode_sizes[0]=}")
        fft_size = list(mode_sizes)
        fft_size[-1] = fft_size[-1] // 2 + 1  # Redundant last coefficient
        fft_dims = list(range(-self.order, 0))
        # print(f'FFT dims: {fft_dims=}, {fft_size=}, {mode_sizes=}')
        ...
        # print("Slices: ",f"{slices_x=}, {slices_w=}")
        # print("Weight full: ", f"{len(self.weight)=}, {self.weight[0].shape=}")
        # print("Weight sliced: ", f"{weight.shape=}")
        # print("Weight sliced 2: ", f"{weight[slices_w].shape=}")
        # print("Out FFT full: ", f"{out_fft.shape=}")
        # print("Out FFT scliced: ", f"{out_fft[slices_x].shape=}")
        if self.output_scaling_factor is not None and output_shape is None:
