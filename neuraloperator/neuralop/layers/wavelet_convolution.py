...
class WaveConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, level, size, wavelet='db4', mode='symmetric', architecture='standard'):
        ...
        if self.architecture in ['euclidean', 'bregman']:
            self.weights1 = nn.Parameter(torch.zeros(in_channels, out_channels, self.modes1))
            self.weights2 = nn.Parameter(torch.zeros(in_channels, out_channels, self.modes1))
        else:
...
class WaveConv2d(nn.Module):
    def __init__(
        self, in_channels, out_channels, level, size, wavelet, mode="symmetric",architecture="standard"
    ):
        ...
        if self.architecture in ["euclidean", "bregman"]:
            self.weights1 = nn.Parameter(
                torch.zeros(in_channels, out_channels, self.modes1, self.modes2)
            )
