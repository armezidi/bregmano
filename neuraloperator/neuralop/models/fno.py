...
class FNO(BaseModel, name='FNO'):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.fno_blocks = FNOBlocks(
            ...
            output_scaling_factor=output_scaling_factor,
            use_mlp=use_mlp,
            use_fno_skip=use_fno_skip,
            mlp_dropout=mlp_dropout,
            mlp_expansion=mlp_expansion,
            non_linearity=non_linearity,
            non_linearity_inv = non_linearity_inv,
            non_linearity_range = non_linearity_range,
    ...
    def add_layer(self):
        """Add a new FNO block to the model"""
        # Add new FNO block
        self.n_layers += 1
        
        # Update FNO blocks
        self.fno_blocks.n_layers += 1
        
        # Add new skip connection if using them
        if self.use_fno_skip:
            self.fno_blocks.fno_skips.append(
                skip_connection(
                    self.hidden_channels,
                    self.hidden_channels,
                    skip_type=self.fno_skip[0],
                    n_dim=self.n_dim,
                )
            )
            if self.architecture == 'bregman':
                self.fno_blocks.fno_skips[-1].weight.data.fill_(0)

        # Add new MLP if using them
        if self.fno_blocks.mlp is not None:
            self.fno_blocks.mlp.append(
                MLP(
                    in_channels=self.hidden_channels,
                    hidden_channels=round(self.hidden_channels * self.fno_blocks.mlp_expansion),
                    dropout=self.fno_blocks.mlp_dropout,
                    n_dim=self.n_dim,
                )
            )
            self.fno_blocks.mlp_skips.append(
                skip_connection(
                    self.hidden_channels,
                    self.hidden_channels,
                    skip_type=self.fno_blocks.mlp_skip,
                    n_dim=self.n_dim,
                )
            )

        # Add new normalization layers if using them
        if self.fno_blocks.norm is not None:
            for _ in range(self.fno_blocks.n_norms):
                if isinstance(self.fno_blocks.norm[0], nn.InstanceNorm1d):
                    self.fno_blocks.norm.append(
                        getattr(nn, f"InstanceNorm{self.n_dim}d")(num_features=self.hidden_channels)
                    )
                elif isinstance(self.fno_blocks.norm[0], nn.GroupNorm):
                    self.fno_blocks.norm.append(
                        nn.GroupNorm(num_groups=1, num_channels=self.hidden_channels)
                    )
                elif isinstance(self.fno_blocks.norm[0], nn.BatchNorm1d):
                    self.fno_blocks.norm.append(
                        getattr(nn, f"BatchNorm{self.n_dim}d")(num_features=self.hidden_channels)
                    )
                elif isinstance(self.fno_blocks.norm[0], AdaIN):
                    self.fno_blocks.norm.append(
                        AdaIN(self.fno_blocks.ada_in_features, self.hidden_channels)
                    )
