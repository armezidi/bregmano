...
from .callbacks import PipelineCallback, EarlyStoppingCallback
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx, sample in enumerate(train_loader):
                ...
                if self.overrides_loss:
                    ...
                    #     print(name, param.data.norm(2).item())
                if regularizer:
                    loss += regularizer.loss
                loss.backward()
                del out
                if self.model.n_layers > 16:                    
                    torch.nn.utils.clip_grad_value_(self.model.parameters(), 1.0)
                    # print(max([param.grad.data.norm(2).item() for param in self.model.parameters()]))
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(),1.0,error_if_nonfinite=False)
                    # with torch.no_grad():
                        # print(loss.item())
                optimizer.step()
                train_err += loss.item()
            ...
            if self.early_stopping_callback and self.early_stopping_callback.early_stop:
                break
